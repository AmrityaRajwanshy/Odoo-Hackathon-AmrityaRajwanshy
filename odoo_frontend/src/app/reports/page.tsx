...
export default function ReportsPage() {
  const { assets, departments, bookings, maintenanceRequests, currentRole } = useApp();
  ...
  // 2. Compute category usage for charts
  const categoriesList = ['Electronics', 'Furniture', 'Vehicles'];
  const categoryCounts = categoriesList.map(cat => ({
    name: cat,
    count: assets.filter(a => a.category === cat).length,
    allocated: assets.filter(a => a.category === cat && a.status === 'Allocated').length
  }));
        ...
        <div className="glass-panel chart-panel">
          ...
          <div className="svg-chart-container">
            <svg viewBox="0 0 400 150" className="trend-svg">
              ...
              {/* Path Area */}
              <path d="M 0 120 Q 80 50 160 90 T 320 30 L 400 60 L 400 150 L 0 150 Z" fill="url(#gradient-area)" />
              
              {/* Path Stroke */}
              <path d="M 0 120 Q 80 50 160 90 T 320 30 L 400 60" fill="none" stroke="var(--accent)" strokeWidth="2.5" />
              
              {/* Data points */}
              <circle cx="0" cy="120" r="4" fill="var(--bg-surface)" stroke="var(--accent)" strokeWidth="2" />
              <circle cx="80" cy="50" r="4" fill="var(--bg-surface)" stroke="var(--accent)" strokeWidth="2" />
              <circle cx="160" cy="90" r="4" fill="var(--bg-surface)" stroke="var(--accent)" strokeWidth="2" />
              <circle cx="320" cy="30" r="4" fill="var(--bg-surface)" stroke="var(--accent)" strokeWidth="2" />
              <circle cx="400" cy="60" r="4" fill="var(--bg-surface)" stroke="var(--accent)" strokeWidth="2" />
            </svg>
          </div>

          <div className="chart-legend" style={{ justifyContent: 'center', gap: '24px' }}>
            <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>Mar: 3 req</span>
            <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>Apr: 8 req</span>
            <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>May: 5 req</span>
            <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>Jun: 12 req</span>
            <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>Jul: {maintenanceRequests.length} req</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid-3 stats-lists-row">
          ...
          <ul className="stats-list">
            <li>
              <div className="list-item-meta">
                <span className="item-title">Conference Room 22</span>
                <span className="item-desc">34 bookings this month</span>
              </div>
              <span className="list-item-val font-bold">Room</span>
            </li>
            <li>
              <div className="list-item-meta">
                <span className="item-title">Ford Transit Cargo Van</span>
                <span className="item-desc">21 dispatch trips logged</span>
              </div>
              <span className="list-item-val font-bold">Van</span>
            </li>
            <li>
              <div className="list-item-meta">
                <span className="item-title">Epson 4K Projector</span>
                <span className="item-desc">18 meeting requests</span>
              </div>
              <span className="list-item-val font-bold">Device</span>
            </li>
          </ul>
        </div>
        ...
        <div className="glass-panel list-panel">
          ...
          <ul className="stats-list">
            <li>
              <div className="list-item-meta">
                <span className="item-title">Ergonomic Mesh Chair (AF-0201)</span>
                <span className="item-desc">Unused in warehouse for 45+ days</span>
              </div>
              <span className="list-item-val text-warning">Idle</span>
            </li>
            <li>
              <div className="list-item-meta">
                <span className="item-title">Dell XPS 15 (AF-0051)</span>
                <span className="item-desc">Unallocated in IT locker for 30 days</span>
              </div>
              <span className="list-item-val text-warning">Idle</span>
            </li>
          </ul>
        ...
        <div className="glass-panel list-panel">
          ...
          <ul className="stats-list">
            <li>
              <div className="list-item-meta">
                <span className="item-title">Apple iPad Pro (AF-0099)</span>
                <span className="item-desc">Condition: Poor | Marked Lost</span>
              </div>
              <span className="list-item-val text-danger">Action</span>
            </li>
            <li>
              <div className="list-item-meta">
                <span className="item-title">Epson Projector (AF-0062)</span>
                <span className="item-desc">In maintenance | Bulb replacement</span>
              </div>
              <span className="list-item-val text-danger">Action</span>
            </li>
          </ul>
