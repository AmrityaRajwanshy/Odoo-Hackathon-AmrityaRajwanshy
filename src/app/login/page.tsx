...
export default function LoginPage() {
  ...
  const { employees, addEmployee, setCurrentRole, setCurrentUser, theme, setTheme } = useApp();
      ...
      if (isLogin) {
        ...
        if (user) {
          if (user.status === 'Inactive') {
            setError('This account is suspended. Contact Administrator.');
            setLoading(false);
            return;
          }
          // Successful login
          ...
          setError('Invalid credentials. Hint: use "sumit@company.com" or sign up.');
          setLoading(false);
        }
      } else {
        // Signup
        ...
        // Add to employees list (defaults to Employee role)
        const newEmp = {
          name,
          email,
          department,
        ...
        const createdUser = { ...newEmp, id: 'temp-' + Math.random().toString(36).substr(2, 9) };
        setTimeout(() => {
  ...
  return (
    <div className="login-split-container">
      ...
      {/* RIGHT PANEL: Copied graphic image mockup */}
      <div className="login-right-panel" style={{ padding: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: theme === 'dark' ? '#111827' : '#EBF2FF' }}>
        <img 
