...
const LineSidebar = ({
  ...
  const tickClass = showMarker
    ? `line-sidebar-tick ${scaleTick ? 'scale-tick' : 'no-scale-tick'}`
    : '';

  const paddingLeftStyle = showMarker
    ? { paddingLeft: `calc(${markerLength}px + ${markerGap}px)` }
    : {};

  return (
    <nav
      className={`line-sidebar-nav ${className}`}
      style={
        {
      ...
      >
        {items.map((label, index) => (
          <li
            ...
            className={`line-sidebar-item ${tickClass} item-${label.toLowerCase().replace(/&/g, '').replace(/\s+/g, '-').replace(/-+/g, '-')}`}
          >
            {showMarker && (
              <span
                aria-hidden="true"
